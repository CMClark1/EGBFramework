...
Sindex <- CAA %>% dplyr::select(a6, a7, a8, a9)
Sindex<-as.matrix(Sindex)
rownames(Sindex)<-seq(1986, 2023, 1)
...
Sindex <- CAA %>% dplyr::select(a5, a6, a7, a8, a9)
Sindex<-as.matrix(Sindex)
...
Sindex <- CAA %>% dplyr::select(a3, a4, a5, a6)
Sindex<-as.matrix(Sindex)
rownames(Sindex)<-seq(1978, 2022, 1)
...
ggsave(here("figures/Survey_SinclairZ.png"), height=10, width=8, units="in")
